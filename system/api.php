...
$stmt = isset($_GET["url"]) && isset($_GET["key"]) && !empty($_GET["url"]) && !empty($_GET["key"]) && filter_var($_GET["url"], FILTER_VALIDATE_URL) && hash_equals($api_settings["key_1"], $_GET["key"]) && hash_equals($config["fingerprint"], create_fingerprint($website_domain, $config["purchase_code"]));
if ($stmt) {
    $host = parse_url($_GET["url"], PHP_URL_HOST);
    $domain = str_ireplace("www.", "", $host);
    $main_domain = get_main_domain($host);
    $_SESSION['video'][$_SESSION["token"]] = $_GET["url"];
    ...
    switch (true) {
        case($domain == "instagram.com"):
            ...
            return_json($download->media_info($_GET["url"]));
            break;
