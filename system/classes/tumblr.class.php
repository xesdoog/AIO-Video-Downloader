...
class tumblr
{
    ...
    function media_info($url)
    {
        ...
        if (isset($match[1][0]) != "") {
            ...
            $video["links"][0]["bytes"] = get_file_size($match[1][0], $this->enable_proxies, false);
            $video["links"][0]["size"] = format_size($video["links"][0]["bytes"]);
            $video["links"][0]["quality"] = "HD";
            $video["links"][0]["mute"] = "no";
            return $video;
        } else {
            preg_match('/src="(.*?).mp4"/', $curl_content, $matches);
            if (count($matches) > 1) {
                $video["links"][0]["url"] = $matches[1] . ".mp4";
                $video["links"][0]["type"] = "mp4";
                $video["links"][0]["bytes"] = get_file_size($matches[1] . ".mp4", $this->enable_proxies, false);
                $video["links"][0]["size"] = format_size($video["links"][0]["bytes"]);
                $video["links"][0]["quality"] = "HD";
                $video["links"][0]["mute"] = "no";
                return $video;
    ...
    function get_thumbnail($curl_content)
    {
        if (preg_match_all('@<meta property="og:image" content="(.*?)" />@si', $curl_content, $match)) {
            ...
            return $match[1][0];
        } else if (preg_match('/src="(.*?).jpg"/', $curl_content, $match)) {
            return $match[1] . ".jpg";
        }
    }
