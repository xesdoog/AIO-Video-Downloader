        ...
        if (!empty($match[1][0])) {
            ...
            foreach ($json["player"]["sources"] as $url) {
                if (preg_match_all("@/(.*?).mp4@si", $url["file"], $match)) {
                    ...
                    $data["links"][$i]["bytes"] = get_file_size($url["file"], $this->enable_proxies, false);
            ...
            $data["links"][0]["bytes"] = get_file_size($json["contentUrl"], $this->enable_proxies, false);
            $data["links"][0]["size"] = format_size($data["links"][0]["bytes"]);
        } else {
            return false;
        }
        return $data;
